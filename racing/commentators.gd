...
# @onready var tts_engines := voice_ids as Array
...
func load_voices() -> PackedStringArray:
...
func _on_race_watcher_lead_changed(new_lead: Horsey, prev_lead: Horsey) -> void:
	var phrase := get_phrase(lead_changed_phrases)\
		.replace("{new}", new_lead.name)\
		.replace("{prev}", prev_lead.name)
	say_phrase(phrase)

func get_phrase(phrases: Array[String]) -> String:
	return phrases.pick_random()\
	.replace("{random}", race.horseys.pick_random().name)\
	.replace("{lead}", race.horseys.front().name)\
	.replace("{last}", race.horseys.back().name)
